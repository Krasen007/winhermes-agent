...
class GatewayRunner:
    ...
    async def stop(self) -> None:
        ...
        for platform, adapter in list(self.adapters.items()):
            try:
                ...
                logger.debug(
                    "[FAIL] %s background-task cancel error: %s", platform.value, e
                )
            try:
                await adapter.disconnect()
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        # If previous session expired and was auto-reset, prepend a notice
        # so agent knows this is a fresh conversation (not an intentional /reset).
        ...
        try:
            ...
            # Stop persistent typing indicator now that agent is done
            try:
                _typing_adapter = self.adapters.get(source.platform)
                if _typing_adapter and hasattr(_typing_adapter, "stop_typing"):
                    await _typing_adapter.stop_typing(
                        source.chat_id,
                        thread_id=source.thread_id,
                    )
            except Exception:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        if args.strip().lower() == "custom":
            ...
            try:
                ...
                if "model" not in user_config or not isinstance(
    ...
    async def _run_background_task(
        ...
        try:
            ...
            if config_toolsets and isinstance(config_toolsets, list):
                ...
                default_toolset = default_toolset_map.get(
                    source.platform, "hermes-telegram"
                )
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if (
            ...
            if ctx.last_prompt_tokens:
                pct = (
                    ctx.last_prompt_tokens / ctx.context_length * 100
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if not git_dir.exists():
            return "[FAIL] Not a git repository -- cannot update."

        hermes_cmd = _resolve_hermes_bin()
        if not hermes_cmd:
            return (
                "[FAIL] Could not locate the `hermes` command. "
        ...
        try:
            ...
            exit_code_path.unlink(missing_ok=True)
            return f"[FAIL] Failed to start update: {e}"
...
async def start_gateway(
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            try:
                _kill_process(existing_pid, force=False)
            ...
            for _ in range(20):
                ...
                _kill_process(existing_pid, force=True)
                _time.sleep(0.5)
            remove_pid_file()
